...
my $shaper	= $Bin.'/shapedata/shaper.list';
my $shaperdiff	= $Bin.'/shapedata/shaper.list.change';
my $shapertmp	= $Bin.'/shapedata/shaper.list.tmp';

open(REAL_LIST, $nas_conf->{'SHAPER_SSHPROG'}.' '.$nas_conf->{'SHAPER_SSHUSER'}.'@'.$nas_conf->{'SHAPER_SSHHOST'}.' sudo /sbin/ipfw table '.$nas_conf->{'SHAPER_IPFWTABLE'}.' list |' );
while (<REAL_LIST>) {
    if (/^(\d+\.\d+\.\d+\.\d+)\/32\s+(\d+)/) {
        $pipeold{$1}=$2;
    } elsif (/^(\d+\.\d+\.\d+\.\d+\/\d+)\s+(\d+)/) {
...
if ($change and not $debug) {
    system('cat '.$shaperdiff.' | '.$nas_conf->{'SHAPER_SSHPROG'}.' '.$nas_conf->{'SHAPER_SSHUSER'}.'@'.$nas_conf->{'SHAPER_SSHHOST'}.' '.$nas_conf->{'SHAPER_REMOTECMD'} );
}
