...
my $sth = $dbm->prepare( "SELECT clients_vlan, hostname, lastuserport, ip, def_trunk, lib, admin_login, admin_pass, sw_id, uplink_port, bw_free, bw_ctl FROM hosts h, models m WHERE h.model_id=m.model_id and manage = 1 and h.visible > 0");
$sth->execute();
while( my $sw = $sth->fetchrow_hashref() )
{
    ...
    my $sw_ports = $dbm->selectall_hashref( "SELECT * FROM swports WHERE sw_id = '$sw->{sw_id}' and type = 1", "port" );
    my $sw_real;
    {
        no strict 'refs';
        my $func = "$sw->{lib}_switch_params";
        $sw_real = &$func( LIB => $sw->{'lib'}, ACT => 'switch_params', IP=>$sw->{ip}, LOGIN=>$sw->{admin_login},PASS=>$sw->{admin_pass}, DEF_TRUNK=>$sw->{def_trunk} );
    }
...
#print Dumper $sw_ports->{$port}, $sw_real->{ports}{$port};
        ...
        my $ports_vlans = $dbm->selectall_hashref( "SELECT * FROM port_vlantag WHERE port_id = '$sw_ports->{$port}{port_id}'", "vlan_id" );
