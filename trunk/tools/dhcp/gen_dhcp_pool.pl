...
sub gen_pool {
	...
	$Q_addip = "INSERT INTO dhcp_addr SET pool_id=".$pool{'pool_id'}.", ip='".$current_ip."', n_ip=INET_ATON('".$current_ip."') ";
	$Q_addip .= " ON DUPLICATE KEY UPDATE pool_id=".$pool{'pool_id'}.", n_ip=INET_ATON(ip)";
	print $Q_addip."\n" if $debug;
